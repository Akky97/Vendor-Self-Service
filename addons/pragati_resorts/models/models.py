            ...
            if discount_amount:
                if discount_amount.program_id.coupon_value:
                    ...
                    if discount_amount.reward_type == 'product':
                        qr.add_data(", Complementary Product :")
...
class LoyaltyGenerateRequest(models.Model):
    ...
    mode = fields.Selection([
        ...
        string='For', default='anonymous'
    )
    customer_ids = fields.Many2many('res.partner', string='Customers')
    customer_tag_ids = fields.Many2many('res.partner.category', string='Customer Tags')
    coupon_qty = fields.Integer('Quantity')
    points_granted = fields.Float('Grant')
    points_name = fields.Char()
    valid_until = fields.Date()
    status = fields.Selection([('draft', 'Draft'), ('cancel', 'Cancel'), ('reject', 'Reject'), ('approve', 'Approve')], default='draft')
